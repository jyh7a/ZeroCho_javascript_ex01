...
var 턴 = true; // true면 내턴, false 면 니턴 둘다 아니면 샘헤밍턴~
...
function 턴액션수행(카드, 데이터, 내턴) {
  ...
  if (적군카드 && 아군.선택카드) { // 턴은 내턴인데 상대 카드클릭햇을때 턴이 끝난 카드가 아니면  상대를 공격
...
function 내영웅생성() {
  나.영웅data.push(카드공장(true, true));
  카드돔연결(나.영웅data[0], 나.영웅, true);
}

function 상대영웅생성() {
  상대.영웅data.push(카드공장(true, false));
  카드돔연결(상대.영웅data[0], 상대.영웅, true);
}

function Card(영웅, 내카드) {
  if (영웅) {
    this.att = Math.ceil(Math.random() * 2);
    this.hp = Math.ceil(Math.random() * 5) + 10;
    this.hero = true;
    this.field = true;
  } else {
...
턴버튼.addEventListener('click', function () {
  ...
  영웅다시그리기(객체);
  턴 = !턴;
  턴 === true ? 턴표시.style.top = "65%" : 턴표시.style.top = "20%";
  if (턴) {
